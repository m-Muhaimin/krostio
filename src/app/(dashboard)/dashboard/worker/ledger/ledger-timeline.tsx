...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  delivery: 'var(--color-link-blue)',
  freelance: 'var(--color-deep-green)',
  marketplace: 'var(--color-signal-orange)',
  flex: 'var(--color-slate-gray)',
}
  ...
  return (
    <section>
      <div className="mb-6 flex items-baseline justify-between">
        <h2 className="text-heading-feature text-ink-black">Detailed entries</h2>
      ...
      {showFilters && (
        <div className="mb-6 flex flex-wrap items-center gap-3">
          <select
            ...
            className="rounded-full border border-hairline px-4 py-2 text-sm outline-none focus:border-ink-black"
          >
          ...
          <div className="overflow-x-auto rounded-md border border-hairline">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-hairline">
                {entries.map((e) => (
                  <tr key={e.id} className="hover:bg-soft-stone transition-colors">
                    <td className="px-4 py-3 text-ink-black whitespace-nowrap">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="flex h-7 w-7 items-center justify-center rounded-sm bg-ink-black text-[10px] font-medium text-white">
                    ...
                    <td className="px-4 py-3">
                      {e.category && (
                        <span
                          ...
                          style={{
                            backgroundColor: CATEGORY_COLORS[e.category] || 'var(--color-slate-gray)',
                          }}
                        >
                          {e.category}
                        </span>
                    ...
                    <td className="px-4 py-3 text-right font-medium text-ink-black">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`text-[10px] uppercase tracking-wide ${
                        e.source === 'plaid' ? 'text-deep-green' : 'text-slate'
                      }`}>
                        {e.source}
                      </span>
                    </td>
